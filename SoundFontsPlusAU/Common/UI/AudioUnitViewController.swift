	...
	*/
	...
	nonisolated public func createAudioUnit(with componentDescription: AudioComponentDescription) throws -> AUAudioUnit {
		return try DispatchQueue.main.sync {
			...
			audioUnit = try SoundFontsPlusAUAudioUnit(componentDescription: componentDescription, options: [])

			guard let audioUnit = self.audioUnit as? SoundFontsPlusAUAudioUnit else {
				log.error("Unable to create SoundFontsPlusAUAudioUnit")
				return audioUnit!
			}
			...
			audioUnit.setupParameterTree(SoundFontsPlusAUParameterSpecs.createAUParameterTree())
